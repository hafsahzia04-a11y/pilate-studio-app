...
import { cn, formatTime, occupancyLabel, occupancyColor, formatCurrency } from "@/lib/utils";
...
interface Session {
  ...
  capacity: number; bookedCount: number; waitlistCount: number;
  spotsLeft: number; isFull: boolean; isWorkshop: boolean;
  workshopPrice: number | null; usesCredits: boolean;
...
export function BookingUI() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/classes?date=${date}&status=scheduled`)
      .then(r => r.json()).then(data => setSessions(Array.isArray(data) ? data : data?.data ?? []))
  ...
  async function book(sessionId: string) {
    ...
    if (data.waitlisted || data.data?.waitlisted) {
      toast("You're on the waitlist 🙏", { icon: "⏳" });
    } else {
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1">
        {days.map((day, i) => (
          <button key={i} onClick={() => setSelectedDay(i)}
            className={cn("flex flex-col items-center px-3.5 py-2 rounded-xl flex-shrink-0 transition-all",
              selectedDay === i ? "bg-sage-500 text-white" : "bg-white text-stone-600 border border-stone-200 hover:border-sage-300")}>
          ...
          {["all", ...categories].map(cat => (
            <button key={cat} onClick={() => setCatFilter(cat)}
              className={cn("px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all flex-shrink-0",
                catFilter === cat ? "bg-sage-500 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200")}>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {s.isWorkshop && s.workshopPrice && (
                    <p className="mt-1.5 text-xs text-amber-700 font-medium">Workshop · {formatCurrency(s.workshopPrice)}</p>
                  )}
                  <div className="mt-3">
